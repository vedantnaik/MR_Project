...
public class Server implements Runnable {
	...
	private static String serverPivs = "";
	private static String mypivs = "";
...
//				System.out.println("Waiting for read");
				...
				if (receivedResult[0].equals("sort")) {
					...
					if(receivedResult[1].equals("start")){
						System.out.println("STAGE 4: Receive Global Pivots");	
						globalPivotON = true;
					}
					...
					if(receivedResult[1].equals("end")){
						...
						for (int i = 1; i < 3; i++) {
							...
							for(int j = 0 ; j < integersToBeSent.get((serverNumb + i) % 3).size(); j++){
								outDist[(serverNumb + i) % 3].writeBytes(integersToBeSent.get((serverNumb + i) % 3).get(j) + "\n");
							}
