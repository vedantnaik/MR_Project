...
# complete and exit
comds="cd ~/Project; java -Xmx5120m -Xms256m -XX:HeapDumpPath=/home/ubuntu/Project/heapdump.hprof -cp ~/Project/DistributedEC2Sorting-0.0.1-SNAPSHOT-jar-with-dependencies.jar server.Server $i $inputBucket $bucketName > log.txt 2>&1"
comds1="cd ~/Project; java -Xmx5120m -Xms256m -XX:HeapDumpPath=/home/ubuntu/Project/heapdump.hprof -cp ~/Project/DistributedEC2Sorting-0.0.1-SNAPSHOT-jar-with-dependencies.jar server.Server "
comds2="$inputBucket $bucketName > log.txt 2>&1"
comdsserver="rm -rf ~/Project/sampleSortPartTemp/; rm ~/Project/sampleSortMyParts/*;cd ~/Project; java -Xmx5120m -Xms256m -XX:HeapDumpPath=/home/ubuntu/Project/heapdump.hprof -cp ~/Project/DistributedEC2Sorting-0.0.1-SNAPSHOT-jar-with-dependencies.jar server.Server 0 $inputBucket $bucketName > log.txt 2>&1"
...
while read line;do
        ...
        if [ $i -eq 0 ]; then
        	...
        	echo "slave instance"
                `ssh -i MyKeyPair.pem ubuntu@$line -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "rm -rf ~/Project/sampleSortPartTemp/; rm ~/Project/sampleSortMyParts/*;cd ~/Project; java -Xmx5120m -Xms256m -XX:HeapDumpPath=/home/ubuntu/Project/heapdump.hprof -cp ~/Project/DistributedEC2Sorting-0.0.1-SNAPSHOT-jar-with-dependencies.jar server.Server $i $inputBucket $bucketName > log.txt 2>&1"` < /dev/null &
        fi
        i=$((i+1))
done < $FILE
